    ...
    view! {
        <div class="card">
            ...
            {move || show_edit_room_modal.get().map(|room| {
                ...
                let (saving, set_saving) = create_signal(false);
                let rid = room.id.clone().unwrap_or_default();
                let handle_room_update = move |ev: leptos::ev::SubmitEvent| {
                    ...
                    spawn_local(async move { wait_for_bridge().await; let _ = update_room_js(rid_val, serde_wasm_bindgen::to_value(&updated_room).unwrap()).await; set_show_edit_room_modal.set(None); load_data(); });
                };
