...
pub async fn fetch_bookings() -> Vec<Booking> {
    ...
    match get_bookings_js().await {
        Ok(js_val) => serde_wasm_bindgen::from_value::<Vec<Booking>>(js_val).unwrap_or_default(),
        Err(_) => vec![],
    }
}
    ...
    let on_check_in = move |ev: leptos::ev::SubmitEvent| {
        ...
        if let (Some(r), Some(c)) = (room_opt, cust_opt) {
            ...
            spawn_local(async move {
                ...
                if let Some(id) = editing_id.get() { let _ = update_booking_js(id, js_val).await; } 
                else { let _ = add_booking_js(js_val).await; }
                set_editing_id.set(None); set_selected_room.set("".to_string()); set_selected_cust.set("".to_string()); load_data();
            });
        }
    };
    ...
    view! {
        <div class="card">
            ...
            <form on:submit=on_check_in class="card" style="background: #f9f9f9;">
                <div class="grid-form">
                    <div style="display: flex; flex-direction: column;"><label>"Select Room"</label><select on:change=move |ev| set_selected_room.set(event_target_value(&ev)) prop:value=selected_room required><option value="">"Choose Room..."</option>{move || rooms.get().into_iter().filter(|r| r.status == "Available" || (editing_id.get().is_some() && Some(r.id.clone().unwrap_or_default()) == Some(selected_room.get()))).map(|r| { let r_id = r.id.clone().unwrap_or_default(); let r_num = r.number.clone(); let r_type = r.room_type.clone(); view! { <option value=r_id>{r_num} " (" {r_type} ")" </option> } }).collect_view()}</select></div>
                    <div style="display: flex; flex-direction: column;"><label>"Select Customer"</label><select on:change=move |ev| set_selected_cust.set(event_target_value(&ev)) required prop:value=selected_cust><option value="">"Choose guest..."</option>{move || customers.get().into_iter().map(|c| { let cid = c.id.clone().unwrap_or_default(); let phone = c.phone.clone(); view! { <option value=cid>{c.full_name.clone()} " (" {phone} ")" </option> } }).collect_view()}</select></div>
                ...
                <button type="submit" style="width: 100%; margin-top: 20px; background-color: #27ae60;">{move || if editing_id.get().is_some() { "Update Stay" } else { "Confirm Check-in" }}</button>
            </form>
            <h3>"Recent Stays"</h3>
            {move || if loading.get() { view! { <p>"Loading..."</p> }.into_view() } else { view! {
                <table>
                    ...
                    <tbody><For each=move || bookings.get() key=|b| b.id.clone().unwrap_or_default() children=move |b| {
                        let b_id = b.id.clone().unwrap_or_default(); let b_edit = b.clone(); let b_del = b.clone(); let b_id_c = b_id.clone();
                        let paid: f64 = b.payments.iter().map(|p| p.amount).sum();
                        view! { 
                            <tr><td>{b.customer_name.clone()}</td><td>{b.room_number.clone()}</td><td>{b.check_in_date.clone()}</td><td><span style=format!("color: {}; font-weight: bold;", if b.total_amount > paid { "#e67e22" } else { "#27ae60" })>"₹" {paid} " / ₹" {b.total_amount}</span></td><td>{move || if confirm_delete_id.get() == Some(b_id_c.clone()) { let b_final = b_del.clone(); view! { <div style="display: flex; gap: 5px; align-items: center;"><span style="font-size: 0.7rem; color: red;">"Sure?"</span><button on:click=move |_| on_delete_final(b_final.clone()) style="padding: 2px 8px; font-size: 0.7rem; background: #e74c3c;">"YES"</button><button on:click=move |_| set_confirm_delete_id.set(None) style="padding: 2px 8px; font-size: 0.7rem; background: #6c757d;">"NO"</button></div> }.into_view() } else { let b_edit_c = b_edit.clone(); let b_id_del = b_id_c.clone(); view! { <div style="display: flex; gap: 5px;"><button on:click=move |_| on_edit(b_edit_c.clone()) style="padding: 5px 10px; font-size: 0.8rem; background: #3498db;">"Edit"</button><button on:click=move |_| set_confirm_delete_id.set(Some(b_id_del.clone())) style="padding: 5px 10px; font-size: 0.8rem; background: #e74c3c;">"Del"</button></div> }.into_view() }}</td></tr> 
                        }
                    } /></tbody>
                </table>
            }.into_view() }}
