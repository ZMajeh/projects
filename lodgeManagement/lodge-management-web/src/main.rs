...
fn Bookings() -> impl IntoView {
    ...
    let on_check_in = move |ev: leptos::ev::SubmitEvent| {
        ...
        if let (Some(r), Some(c)) = (room_opt, cust_opt) {
            let new_booking = NewBooking { room_id: r_id, customer_id: c_id, customer_name: c.full_name, room_number: r.number, check_in_date: check_in.get(), check_out_date: check_out.get(), status: "Checked-In".to_string(), };
            spawn_local(async move { wait_for_bridge().await; let js_val = serde_wasm_bindgen::to_value(&new_booking).unwrap(); match add_booking_js(js_val).await { Ok(_) => { load_data(); } Err(e) => logging::error!("Booking Error: {:?}", e), } });
        }
    };
