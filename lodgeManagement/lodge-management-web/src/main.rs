...
fn Bookings() -> impl IntoView {
    ...
    let on_check_in = move |ev: leptos::ev::SubmitEvent| {
        ...
        let room = rooms.get_untracked().into_iter().find(|r| r.id.as_deref() == Some(&r_id));
        let cust = customers.get_untracked().into_iter().find(|c| c.id.as_deref() == Some(&c_id));

        if let (Some(r), Some(c)) = (room, cust) {
        ...
        <div class="card">
            ...
            <form on:submit=on_check_in class="card" style="background: #f9f9f9;">
                <div class="grid-form">
                    <div style="display: flex; flex-direction: column;">
                        <label>"Select Room (Available Only)"</label>
                    ...
                    <div style="display: flex; flex-direction: column;">
                        ...
                        <select on:change=move |ev| set_selected_cust.set(event_target_value(&ev)) prop:value=selected_cust required>
                            ...
                            <For each=move || customers.get() 
                                 ...
                                 children=|c| view! { <option value=c.id.clone()>{c.full_name.clone()} {if c.verified { " (Verified)" } else { " (Unverified)" }}</option> } />
                        </select>
                    </div>
                    <div style="display: flex; flex-direction: column;">
                        <label>"Check-in Date"</label>
                        <input type="date" on:input=move |ev| set_check_in.set(event_target_value(&ev)) prop:value=check_in required />
                    </div>
                    <div style="display: flex; flex-direction: column;">
                        <label>"Check-out Date (Estimate)"</label>
            ...
            {move || if loading.get() { view! { <p>"Loading..."</p> }.into_view() } else { view! {
                <table>
                    ...
                    <tbody>
                        <For each=move || bookings.get() key=|b| b.id.clone().unwrap_or_default() children=|b| view! {
                            <tr>
